...
const generateTcId = () => `TC-A-SO${Math.floor(Math.random() * 900 + 100)}`;
...
const INITIAL_FORM = {
  ...
  actualBehavior:   "",
  environment:      "",
  screenshots:      [],   // unified: array of { url: string, file?: File }
};
...
const AddBugReport = ({ open, onClose, onSave, editingBug = null, loading = false }) => {
  ...
  useEffect(() => {
    ...
    if (editingBug) {
      setFormData({
        tcId:             editingBug.tcId             || editingBug.id || "",
        ...
        // Normalise existing screenshots to { url } objects
        screenshots: (editingBug.screenshots || []).map((s) =>
          typeof s === "string" ? { url: s } : s
        ),
      });
    } else {
      setFormData({ ...INITIAL_FORM, tcId: generateTcId() });
    }
    setErrors({});
  }, [editingBug, open]);
  ...
  // ── Add screenshots (one or many files) ────────────────────────────────
