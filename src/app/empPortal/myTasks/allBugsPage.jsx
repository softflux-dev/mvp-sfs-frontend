...
// Supports both raw (new, in_progress) and formatted (New, In Progress) values
const TASK_STATUS_CONFIG = {
  ...
  // formatted fallbacks
  "New":          { label: "New",          bg: "#2B6EFF1A", color: "#2B6EFF" },
  "In Progress":  { label: "In Progress",  bg: "#FF972F1A", color: "#FF972F" },
  "Under Review": { label: "Under Review", bg: "#9E9E9E1A", color: "#9E9E9E" },
  "Completed":    { label: "Completed",    bg: "#04C3731A", color: "#04C373" },
};
...
const AllBugsPage = () => {
  ...
  const taskPriority = location.state?.taskPriority || "";

  const {
    bugs, loading, actionLoading, createBug, updateBug, deleteBug,
  ...
  const handleDeleteBug = async (bug) => { await deleteBug(bug._id || bug.id); };

  const handleSaveBug = async (data) => {
    if (editingBug) await updateBug(editingBug._id || editingBug.id, data);
    else            await createBug(data);
  };
  ...
  // Resolve task status — normalize to lowercase for lookup
  const statusKey    = taskStatus?.toLowerCase?.().replace(/ /g, "_") || "";
  const statusCfg    = TASK_STATUS_CONFIG[taskStatus] || TASK_STATUS_CONFIG[statusKey] || { label: taskStatus || "—", bg: "#F5F5F5", color: "#757575" };

  // Resolve priority — normalize to lowercase
  const priorityKey  = taskPriority?.toLowerCase?.() || "";
  const priorityCfg  = PRIORITY_CONFIG[priorityKey] || { label: taskPriority || "—", bg: "#F5F5F5", color: "#757575" };
          ...
          <Box display="flex" alignItems="center" gap={1} flexWrap="wrap">
            ...
            {moduleName && (
              <>
                ...
                <Typography fontSize="13px" color="text.secondary">{moduleName}</Typography>
              </>
            )}
            {taskPriority && (
              <Chip
                ...
                size="small"
                sx={{ height: "22px", fontSize: "11px", fontWeight: 600, px: 0.5, borderRadius: "8px", backgroundColor: priorityCfg.bg, color: priorityCfg.color }}
              />
            )}
          </Box>
        </Box>
        ...
        <Box sx={{ backgroundColor: "#fff", borderRadius: "20px", p: 2.5 }}>
          ...
          <Box display="flex" gap={2} flexWrap="wrap" alignItems="center" mb={2.5}>
            ...
            <Box sx={{ flex: "0 0 155px" }}>
              <CustomSelect value={severityFilter} onChange={(e) => setSeverityFilter(e.target.value)} fullWidth height="45px" inputBgColor="#F5F5F5">
                {SEVERITY_OPTIONS.map((o) => <MenuItem key={o.value} value={o.value}>{o.label}</MenuItem>)}
              </CustomSelect>
            </Box>
            <Box sx={{ flex: "0 0 155px" }}>
              <CustomSelect value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} fullWidth height="45px" inputBgColor="#F5F5F5">
            ...
            <Grid container spacing={2}>
              {filtered.map((bug) => (
                <Grid key={bug._id || bug.id} size={{ xs: 12, sm: 6, md: 4 }}>
                  <BugCard bug={bug} onView={handleViewBug} onEdit={handleEditBug} onDelete={handleDeleteBug} />
                </Grid>
              ))}
            </Grid>
          ) : (
            <Box sx={{ p: 6, textAlign: "center" }}>
              <Typography fontSize="14px" color="text.secondary">No bug reports found.</Typography>
            </Box>
          )}
